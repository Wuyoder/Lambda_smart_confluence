...
func (s *CompletionObj) GetOpenAIResp(ctx context.Context) ([]string, int32) {
	...
	if s.IsChatMode {
		resp := GetChatCompletions(ctx, s.Payload, s.Query, deployID)
...
func GetChatCompletions(ctx context.Context, payload string, query string, deployID string) azopenai.GetChatCompletionsResponse {
